...
volatile uint32_t motorTaskAliveFlag = 0;

const float Kp = 0.00010f;  // Proportional gain
const float Ki = 0.0000006f;  // Integral gain
const float TIMER_INTERVAL = 0.01f;  // Timer interval in seconds (assuming 10ms here)
const float MAX_DELTA_DUTY_NORMAL = 0.01f;  // Max change in duty cycle under normal conditions per interval
const float MAX_DELTA_DUTY_ESTOP = 0.02f;  // Max change in duty cycle under Estop per interval
const int INTEGRAL_MAX = 500;
...
RectangularButton(g_sNext, 0, 0, 0, &g_sKentec320x240x16_SSD2119, 270, 190,
    ...
    Slider(g_sSliderLimitsHigh, 0, 0, 0,
                 &g_sKentec320x240x16_SSD2119, 180, 150, 110, 30, 0, 650, 650,
                 (SL_STYLE_FILL | SL_STYLE_BACKG_FILL | SL_STYLE_OUTLINE |
                  SL_STYLE_TEXT | SL_STYLE_BACKG_TEXT),
                 ClrGray, ClrBlack, ClrSilver, ClrWhite, ClrWhite,
                 &g_sFontCm20, "650", 0, 0, OnSliderLimitHigh);

    Slider(g_sSliderAcc, 0, 0, 0,
                 &g_sKentec320x240x16_SSD2119, 180, 100, 110, 30, 0, 6000, 6000,
                 (SL_STYLE_FILL | SL_STYLE_BACKG_FILL | SL_STYLE_OUTLINE |
                  SL_STYLE_TEXT | SL_STYLE_BACKG_TEXT),
                 ClrGray, ClrBlack, ClrSilver, ClrWhite, ClrWhite,
                 &g_sFontCm20, "6000", 0, 0, OnSliderLimitAcc);
...
// Current limit for motor in mA
int32_t limitHigh = 650;
...
void OnIntroPaint(void)
{
    ...
    GrStringDraw(&sContext, "Welcome this is assignment 2", -1,
                 0, 32, 0);
    ...
    GrStringDraw(&sContext, "Page 2: Motor data plotting.", -1, 0,
                 110, 0);
    GrStringDraw(&sContext, "Page 3: Sensor control", -1, 0,
                 128, 0);
    GrStringDraw(&sContext, "Page 4: Sensor data plotting.", -1, 0,
                 146, 0);
    GrStringDraw(&sContext, "Emergy stop when limits reached", -1, 0,
                 164, 0);
}
...
extern SemaphoreHandle_t xCurrentReadSemaphore;
...
SemaphoreHandle_t xPIDControllerSemaphore;
SemaphoreHandle_t xMotorRunSemaphore;
// Mutex for I2C0
...
/*-----------------------------------------------------------*/
// Called by Main to launch all tasks
void vLaunchTasks( void );
/*-----------------------------------------------------------*/
void vLaunchTasks( void )
{
...
void plotMotorSpeed(uint32_t speed)
{
        if(MotorData)
        {
            GrContextFontSet(&sContext, &g_sFontCm20);
...
{       
    ...
    {   
        ...
        if(!DEMO)
        {
            ...
            {
                if((( xCurrentvalueRecv.value >= limitHigh)) && motorState)
                {
                    EXTERNAL_SET_ESTOP();
                    motorState = false;
                    if(g_ui32Panel == 1)
            ...
            if((xQueueReceive( xOPT3001Queue, &( xOPT3001Message ), 0 ) == pdPASS))
            {
            ...
            if((xQueueReceive( xBMI160Queue, &( xBMI160Message), 0 ) == pdPASS))
            {
...
static void prvMotorPrint( void *pvParameters ){
    while (1){
        ...
        UARTprintf("RPM MOTOR SET: %d \n", motor_state.set_rpm);
        UARTprintf("RPM SCREEN SET: %d \n", (int)motor_state_external.set_rpm);
...
static void vCurrentRead( void *pvParameters )
{
    ...
    for( ;; )
    {
        ...
        if (xSemaphoreTake(xCurrentReadSemaphore, portMAX_DELAY) == pdTRUE)
        {
        ...
        {
            UARTprintf("Semaphore Error current task\n");
        }
    }
}
...
void vRPMRead( void *pvParameters )
{
    ...
    for( ;; )
    {
        ...
        motor_state.current_rpm = xRPMvalueSend.value;
    ...
    for( ;; )
    {
        ...
        if (xSemaphoreTake(xBMI160ReadSemaphore, portMAX_DELAY) == pdTRUE)
        {
            ...
            if(success)
            {
                ...
                filtered_accel = (filtered_accel * 9.81)/ 163.84;

                // // Publish to Queue
                xBMI160Message.ulfilteredAccel = filtered_accel;
                //UARTprintf("BMI160 Queue send\n");
...
void xTimer3AHandler( void )
{
    ...
    xSemaphoreGiveFromISR(xCurrentReadSemaphore, &xHigherPriorityTaskWoken);
    portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
